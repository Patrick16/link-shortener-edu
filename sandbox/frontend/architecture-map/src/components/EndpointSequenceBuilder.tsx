...
import type { EndpointDefinition } from '../types/controlApi'
...
// same way it would if a real user did things in a different order.
    ...
    <div className="endpoint-sequence-builder">
      ...
      <ol className="endpoint-sequence-list">
        {sequence.map((id, index) => {
          ...
          const hints = ep ? [ep.consumes.length > 0 && `needs: ${ep.consumes.join(', ')}`, Object.keys(ep.produces).length > 0 && `produces: ${Object.keys(ep.produces).join(', ')}`].filter(Boolean) : []
          return (
            <li key={`${id}-${index}`} className="endpoint-sequence-step">
              <span className="endpoint-sequence-step-index">{index + 1}</span>
              <span className="endpoint-sequence-step-label">{ep ? `${ep.serviceId}: ${ep.method} ${ep.pathTemplate}` : id}</span>
